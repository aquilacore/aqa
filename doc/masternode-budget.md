...
Aqa now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<aqa_address\> \<monthly_payment_aqa\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ AQA, _12_ times over the course of a year totaling _24000_ AQA.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<aqa_address\> \<monthly_payment_aqa\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` AQA.
 ...
 * vote-many          - Vote on a Aqa initiative
 * vote-alias         - Vote on a Aqa initiative
 * vote               - Vote on a Aqa initiative/budget
