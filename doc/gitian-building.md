...
*Setup instructions for a gitian build of AQA using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AQA
...
Clone the git repositories for aqa and gitian and then checkout the aqa version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aqa-crypto/aqa.git
cd aqa
...
Follow the instructions in [doc/release-process.md](release-process.md) in the aqa repository
...
To build AQA (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the aqa repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aqa/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/aqa-crypto/aqa
...
For example:
```bash
URL=https://github.com/aquilacore/aqa.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit aqa=${COMMIT} --url aqa=${URL} ../aqa/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aqa=${COMMIT} --url aqa=${URL} ../aqa/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aqa=${COMMIT} --url aqa=${URL} ../aqa/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the steps in the build process as normal.
    ...
    gpg: skipped "aqacoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aqa-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aqa-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/aqa-build.assert
```
...
[aqa/gitian.sigs](https://github.com/aqa-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
