...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in aqa\n"
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of aqa addresses\n"
            "     \"aqaaddress\"   	 	(string) aqa address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
...
/** Comparison function for sorting the getchaintaqa heads.  */
...
Value getchaintaqa(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getchaintaqa\n"
            "Return information about all known taqa in the block tree,"
            ...
            "\nExamples:\n" +
            HelpExampleCli("getchaintaqa", "") + HelpExampleRpc("getchaintaqa", ""));

    /* Build up a list of chain taqa.  We start with the list of all
       known blocks, and successively remove blocks that appear as pprev
       of another block.  */
    std::set<const CBlockIndex*, CompareBlocksByHeight> setTaqa;
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex)
        setTaqa.insert(item.second);
    BOOST_FOREACH (const PAIRTYPE(const uint256, CBlockIndex*) & item, mapBlockIndex) {
    ...
    setTaqa.insert(chainActive.Tip());

    /* Construct the output array.  */
    Array res;
    BOOST_FOREACH (const CBlockIndex* block, setTaqa) {
        Object obj;
