...
#define DEV_FEE_BLOCK_ACTIVATION 75000
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    double devfeePercent = nBlockHeight >= DEV_FEE_BLOCK_ACTIVATION ? 0.40 : 0.00;
        ...
        BOOST_FOREACH (CTxOut out, txNew.vout) {
            if(out.scriptPubKey == developerfeescriptpubkey) {
                if(out.nValue >= requiredDeveloperPayment) {
                    foundDevFee = true;
		    LogPrintf("Developer-Fee Payment found! Thanks for supporting AQA!");
                }
            }
	}
